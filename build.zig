...
pub fn build(b: *std.Build) !void {
    ...
    if (wasm) {
        if (b.lazyDependency(wasmtimeDep(target.result), .{})) |wasmtime| {
            ...
            if (shared) lib.root_module.linkSystemLibrary("wasmtime", .{ .preferred_link_mode = .dynamic }) else lib.root_module.linkSystemLibrary("wasmtime", .{ .preferred_link_mode = .static });
        }
    }
